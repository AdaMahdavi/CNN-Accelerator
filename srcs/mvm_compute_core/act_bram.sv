...
/* DESIGN NOTE: Activation BRAM: RAMB36E2 in Simple Dual-Port mode
   Port A = write (load phase, driven by testbench/PS later)
   Port B = read  (control FSM reads during compute)

   Data layout: 64-bit wide, 512 deep
   Address 0 = activation elements [3:0]  (lower 64 bits)
   Address 1 = activation elements [7:4]  (upper 64 bits)
...
module act_bram #(
    ...
    input  wire [63:0]  i_wr_data,     // 4 elements × 16b per write

    // Read port (control FSM)
    input  wire           i_rd_en,
    input  wire [8:0]   i_rd_addr,
    output wire [63:0]  o_rd_data      // 4 elements × 16b per read
);
