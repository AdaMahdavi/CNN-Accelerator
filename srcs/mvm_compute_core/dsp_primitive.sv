...
module dsp48e2_instance #(

    parameter int DATA_W  =  16,
    parameter int ACC_W   =  48 //--- just setting it to default max size for 
)(
    input  wire                    i_clk,
    input  wire                    i_rst,
    input  wire [DATA_W - 1 : 0]     i_a,
    input  wire [DATA_W - 1 : 0]     i_b,
    input  wire [ ACC_W - 1 : 0]  i_pcin,
    output reg  [ ACC_W - 1 : 0] o_pcout,
    output reg  [ ACC_W - 1 : 0]     o_p

);

localparam int A_W = 30;
localparam int B_W = 18;

localparam INMODE  =     5'b00000;
localparam OPMODE  = 9'b000010101;
localparam ALUMODE =      4'b0000;

//--- dsp signals: padding remaining bits by extending sign bits
logic a_sign;
logic b_sign;
...
assign a_sign = i_a [DATA_W - 1];
assign b_sign = i_b [DATA_W - 1];

assign dsp_a = {{(A_W - DATA_W){a_sign}}, i_a[ DATA_W  - 1 : 0]}; 
assign dsp_b = {{(B_W - DATA_W){b_sign}}, i_b[ DATA_W  - 1 : 0]};
...
DSP48E2 #(
   ...
   .IS_OPMODE_INVERTED(9'b00000000), // Optional inversion for OPMODE
...
DSP48E2_inst (
   ...
   .RSTA                    (i_rst),                     // 1-bit input: Reset for AREG
   .RSTALLCARRYIN            (1'b0),   // 1-bit input: Reset for CARRYINREG
   .RSTALUMODE               (1'b0),         // 1-bit input: Reset for ALUMODEREG
   .RSTB                    (i_rst),                     // 1-bit input: Reset for BREG
   .RSTC                     (1'b0),                     // 1-bit input: Reset for CREG
   .RSTCTRL                  (1'b0),               // 1-bit input: Reset for OPMODEREG and CARRYINSELREG
   .RSTD                     (1'b0),                     // 1-bit input: Reset for DREG and ADREG
   .RSTINMODE                (1'b0),           // 1-bit input: Reset for INMODEREG
   .RSTM                    (i_rst),                     // 1-bit input: Reset for MREG
   .RSTP                    (i_rst)                      // 1-bit input: Reset for PREG
);
